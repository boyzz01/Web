...
class User extends REST_Controller {
    ...
    public function token_post($id)
    {
          $device_token = $this->post('device_token');
         if($this->M_Users->updatetoken($id,$device_token))
            {
                $data = [
                    'status' =>true,
                    'pesan' =>'Token Berhasil Disimpan'
                ];
                $this->set_response($data, REST_Controller::HTTP_OK); 
            }else{
                $data = [
                    'status' =>false,
                    'pesan' =>'Token Gagal Disimpan'
                ];
    ...
    public function register_post(){
        ...
        if($this->M_Users->is_exist(null,'username',$username) > 0){
            ...
            $this->set_response($data, REST_Controller::HTTP_OK); 
        } else if($this->M_Users->is_exist(null,'email',$email) > 0){
            $data = [
                'status' =>false,
        ...
        else if($this->M_Users->is_exist(null,'phone',$phone) > 0){
            $data = [
                'status' =>false,
